...
## Zarioを選ぶ理由（他ライブラリとの比較）

Winston や Pino のような有名なロギングライブラリは広く使われていますが、Zario は**高速なロギング**と、モダンなマイクロサービスで求められる**耐障害性（レジリエンス）パターン**を、追加のライブラリなしで両立するために設計されています。

### 機能比較表

| 機能 | Zario 🌟 | Pino 🌲 | Winston 🪵 |
| :--- | :---: | :---: | :---: |
| **実行時依存関係** | **0 (依存関係ゼロ)** | 8個以上の依存パッケージ | 10個以上の依存パッケージ |
| **レジリエンス機能** | **標準搭載** (Retry, Circuit Breaker, DLQ) | カスタムストリームの実装が必要 | サードパーティ製ライブラリが必要 |
| **軽量性・Tree-shaking** | **対応** (`zario/logger` で軽量 ~12KB バンドル) | 不可 / エッジ環境でのビルドが困難 | 不可 / パッケージサイズ大 |
| **プラグイン可能非同期キュー**| **対応** (組み込み + カスタムキュー) | 未対応 (Worker Thread を利用) | 未対応 (同期 / ストリーム書き込み) |
| **構造化ログ・エンリッチャー**| **標準搭載** (Pipeline & OTelコンテキスト) | カスタムプラグインが必要 | Formats で実装が必要 |
| **OpenTelemetryの統合** | **ネイティブ** (トレース / スパン自動追跡) | 外部インテグレーションが必要 | 外部インテグレーションが必要 |
